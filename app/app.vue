...
const [{ data: navigation }, { data: files }] = await Promise.all([
  useAsyncData('navigation', () => {
    return Promise.all([
      ...
      queryCollectionNavigation('docsv4', ['titleTemplate']).then(data => {console.log("queryCollectionNavigation('docsv4', ['titleTemplate'])", data); return data[0]?.children}),
...
const [{ data: navigation2 }, { data: files2 }] = await Promise.all([
  useAsyncData('navigation2', () =>
      queryCollectionNavigation('nuxtcontent', ['titleTemplate']).then(data => {console.log("queryCollectionNavigation nuxtcontent", data); return data[0]?.children})
  , {
    // transform: data => data.flat(),
    // watch: [version]
  }),
...
console.log('navigation2', navigation2.value, 'files2', files2.value);
